...
RUN cd $LANDIS_FOLDER \
  ...
  && ./downloadSpecificGitCommitAndFolder.sh https://github.com/LANDIS-II-Foundation/Extension-PnET-Succession 5e7e5f95d6a1f47b5a8b62cd92c4598aa0b85d78 /src \
  ...
  && ./downloadSpecificGitCommitAndFolder.sh https://github.com/LANDIS-II-Foundation/Extension-PnET-Succession 5e7e5f95d6a1f47b5a8b62cd92c4598aa0b85d78 /deploy \
  ...
  && cd $LANDIS_FOLDER/Extension-PnET-Succession && wget https://raw.githubusercontent.com/LANDIS-II-Foundation/Extension-PnET-Succession/refs/heads/master/deploy/installer/PnET-Succession.txt \
...
## Add Mamba
RUN curl -sL https://micro.mamba.pm/api/micromamba/linux-64/latest | tar xvj bin/micromamba && \
...
RUN micromamba create -y -n medlands -f environment.yml

# 2. Install pip packages using 'micromamba run' and suppress root warning
# The warning happens because 'micromamba run' still executes as root in this context
RUN micromamba run -n medlands pip install grass-session --root-user-action=ignore

# 3. Set PATH so the container uses the 'landis' environment by default
# Adjust the path below to match your actual installation prefix
ENV PATH=/usr/local/share/mamba/envs/medlands/bin:$PATH
RUN micromamba shell init -s bash --root-prefix /usr/local/share/mamba   

# Set the entrypoint to use the environment
CMD ["bash"] 
# Or specifically run your app: CMD ["micromamba", "run", "-n", "landis", "python", "app.py"]   